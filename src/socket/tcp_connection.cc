#include "uringpp/socket/tcp_connection.h"
...
#include "uringpp/socket/channel.h"
...
tcp_connection::connect_awaitable::connect_awaitable(
  ...
  for (p = servinfo; p != nullptr; p = p->ai_next) {
    ...
    if (fd == -1) {
      UCXPP_LOG_ERROR("failed to create socket: %s(errno=%d)", strerror(errno),
                      errno);
      continue;
    }
    ...
    try {
      ...
      UCXPP_LOG_ERROR("%s", e.what());
      continue;
    }
    rc_ = ::connect(channel->fd(), p->ai_addr, p->ai_addrlen);
    if (rc_ < 0) {
      if (errno != EINPROGRESS) {
        UCXPP_LOG_ERROR("failed to connect: %s(errno=%d)", strerror(errno),
                        errno);
        continue;
      }
    }
...
tcp_connection::connect_awaitable::await_resume() {
  ...
  UCXPP_LOG_DEBUG("fd %d connected", channel_->fd());
