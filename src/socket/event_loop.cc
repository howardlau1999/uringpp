#include "uringpp/socket/event_loop.h"
...
void event_loop::register_channel(std::shared_ptr<channel> channel,
                                  struct epoll_event *event) {
  assert(epoll_fd_ > 0);
  UCXPP_LOG_TRACE("epoll add fd=%d events=%s", channel->fd(),
...
event_loop::~event_loop() {
  if (close_event_fd_ > 0) {
    if (auto rc = ::close(close_event_fd_); rc != 0) {
      UCXPP_LOG_ERROR("failed to close event fd %d: %s (errno=%d)",
                      close_event_fd_, strerror(errno), errno);
    } else {
      UCXPP_LOG_TRACE("closed event fd %d", close_event_fd_);
    }
  }
  if (epoll_fd_ > 0) {
    if (auto rc = ::close(epoll_fd_); rc != 0) {
      UCXPP_LOG_ERROR("failed to close epoll fd %d: %s (errno=%d)", epoll_fd_,
                      strerror(errno), errno);
    } else {
      UCXPP_LOG_TRACE("closed epoll fd %d", epoll_fd_);
    }
  }
}
